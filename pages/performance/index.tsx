...
const Performance: FC = () => {
  ...
  const [fromDate, setFromDate] = React.useState<Dayjs | null>(dayjs((new Date(Date.now() - (3600 * 1000 * 24 * 7))).toISOString()));
  const [untilDate, setUntilDate] = React.useState<Dayjs | null>(dayjs((new Date()).toISOString()));
  ...
  const handleRunReport = () => {
    ...
    setShowSpinner(true);
    fetch(`/api/boostiny/performance?fromDate=${fromDate?.toISOString()}&untilDate=${untilDate?.toISOString()}`).then(response => {
  ...
  return (
    <Box component="div">
      <Box component="div" className="flex flex-row justify-center items-center">
        <MaterialUIPicker label="from" handleChange={handleFromDateChange} value={fromDate} />
        <MaterialUIPicker label="until" handleChange={handleUntilDateChange} value={untilDate} />
        <Button variant="outlined" onClick={handleRunReport}>Run Report</Button>
      </Box>
